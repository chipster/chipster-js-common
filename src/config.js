"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var logger_1 = require("./logger");
var fs = require('fs');
var YAML = require('yamljs');
var logger = logger_1.Logger.getLogger(__filename);
var ROOT_PATH = '../../';
var DEFAULT_CONF_PATH = 'conf/chipster-defaults.yaml';
var KEY_CONF_PATH = 'conf-path';
var VARIABLE_PREFIX = 'variable-';
var Config = /** @class */ (function () {
    function Config() {
        this.variables = new Map();
        this.defaultConfPath = ROOT_PATH + DEFAULT_CONF_PATH;
        if (!fs.existsSync(this.defaultConfPath)) {
            throw new Error('default config file not found: ' + this.defaultConfPath);
        }
        var allDefaults = this.readFile(this.defaultConfPath);
        for (var key in allDefaults) {
            if (key.startsWith(VARIABLE_PREFIX)) {
                this.variables.set(key.replace(VARIABLE_PREFIX, ''), allDefaults[key]);
            }
        }
        this.confPath = ROOT_PATH + this.getDefault(KEY_CONF_PATH);
        if (!fs.existsSync(this.confPath)) {
            this.confPath = null;
            if (!Config.confFileWarnShown) {
                logger.warn('configuration file ' + this.confPath + ' not found, using defaults');
                Config.confFileWarnShown = true;
            }
            else {
                // swallow
            }
        }
    }
    Config.prototype.get = function (key) {
        var value;
        if (this.confPath) {
            var confFile = this.readFile(this.confPath);
            if (confFile) {
                value = confFile[key];
            }
        }
        if (!value) {
            value = this.getDefault(key);
        }
        if (!value) {
            throw new Error('configuration key ' + key + ' not found');
        }
        return value;
    };
    Config.prototype.getDefault = function (key) {
        var template = this.readFile(this.defaultConfPath)[key];
        this.variables.forEach(function (variableValue, variableKey) {
            template = template.replace('\{\{' + variableKey + '\}\}', variableValue);
        });
        return template;
    };
    Config.prototype.readFile = function (filePath) {
        return YAML.load(filePath);
    };
    Config.KEY_URL_BIND_TYPE_SERVICE = 'url-bind-type-service';
    Config.KEY_URL_ADMIN_BIND_TYPE_SERVICE = 'url-admin-bind-type-service';
    Config.KEY_URL_INT_SERVICE_LOCATOR = 'url-int-service-locator';
    Config.confFileWarnShown = false;
    return Config;
}());
exports.Config = Config;
